...
app.get("/sse", async (req, res) => {
  ...
  // Create a fresh server instance per connection to avoid "Already connected" errors
      ...
      tools: [
        {
          ...
          inputSchema: {
            type: "object",
            properties: {
              clinical_note: { type: "string", description: "The raw clinical text or patient chart." }
            },
  ...
  mcpServer.setRequestHandler(CallToolRequestSchema, async (request) => {
    if (request.params.name === "run_aegis_pipeline") {
      try {
        ...
        const result = await runAegisPipeline({ rawText: clinical_note });
        const g = result.agents.guardian?.output || {};
        const s = result.agents.sentinel?.output || {};
        const c = result.agents.compliance?.output || {};
        const a = result.agents.advocate?.output || {};
...
Status: ${c.overallStatus || "N/A"}
Summary: ${c.summary || "N/A"}

## Advocate (Prior Authorization)
Status: ${a.status || "N/A"}
Recommendation: ${a.summary || "N/A"}

## Final Arbiter Decision
Risk Level: ${arb.overallRisk || "N/A"}
Summary: ${arb.summary || result.summary?.executiveSummary || "N/A"}
...
`;
  ...
  // Try to read session ID from query params if client sends it there
...
app.post("/message", async (req, res) => {
  // Prompt Opinion might not send x-session-id. We fallback to query param or latest transport.
...
const fs = require("fs");
const AUDIT_DIR = process.env.VERCEL ? "/tmp/data" : path.join(__dirname, "..", "data");
try {
